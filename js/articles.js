// ─── Stirio — Article of the Day ──────────────────────────────────
// Rotating daily article sourced from the Wiki catalog + fichas. All
// content comes from i18n keys that already exist in 5 languages, so
// language changes re-render the article instantly.
//
// Exposes: window.stArticles = { getArticleOfTheDay(), resolveArticle(entry), POOL }

(function () {
  // Curated pool of articles with full 5-language coverage in i18n/*.json.
  // `cat/art` map to `wiki.art.<cat>.<art>.*` keys.
  const POOL = [
    // ── TÉCNICAS ──────────────────────────────────────────────
    { id: 'tech-shake',     type: 'technique', cat: 'techniques', art: 'shake',     emoji: '🫨', color: '#3498db' },
    { id: 'tech-stir',      type: 'technique', cat: 'techniques', art: 'stir',      emoji: '🥄', color: '#3498db' },
    { id: 'tech-muddle',    type: 'technique', cat: 'techniques', art: 'muddle',    emoji: '🪵', color: '#3498db' },
    { id: 'tech-fat-wash',  type: 'technique', cat: 'techniques', art: 'fat-wash',  emoji: '🧈', color: '#3498db' },
    { id: 'tech-dry-shake', type: 'technique', cat: 'techniques', art: 'dry-shake', emoji: '🥚', color: '#3498db' },
    { id: 'tech-infusion',  type: 'technique', cat: 'techniques', art: 'infusion',  emoji: '🌶️', color: '#3498db' },
    { id: 'tech-strain',    type: 'technique', cat: 'techniques', art: 'strain',    emoji: '🫗', color: '#3498db' },
    { id: 'tech-layer',     type: 'technique', cat: 'techniques', art: 'layer',     emoji: '🌈', color: '#3498db' },
    { id: 'tech-blend',     type: 'technique', cat: 'techniques', art: 'blend',     emoji: '🌪️', color: '#3498db' },
    { id: 'tech-build',     type: 'technique', cat: 'techniques', art: 'build',     emoji: '🧱', color: '#3498db' },
    // ── DESTILADOS ────────────────────────────────────────────
    { id: 'sp-whisky',   type: 'spirit', cat: 'spirits', art: 'whisky',  emoji: '🥃', color: '#d35400' },
    { id: 'sp-gin',      type: 'spirit', cat: 'spirits', art: 'gin',     emoji: '🌿', color: '#27ae60' },
    { id: 'sp-rum',      type: 'spirit', cat: 'spirits', art: 'rum',     emoji: '🏝️', color: '#b9770e' },
    { id: 'sp-vodka',    type: 'spirit', cat: 'spirits', art: 'vodka',   emoji: '❄️', color: '#3498db' },
    { id: 'sp-tequila',  type: 'spirit', cat: 'spirits', art: 'tequila', emoji: '🌵', color: '#d4ac0d' },
    { id: 'sp-mezcal',   type: 'spirit', cat: 'spirits', art: 'mezcal',  emoji: '🔥', color: '#a04000' },
    { id: 'sp-brandy',   type: 'spirit', cat: 'spirits', art: 'brandy',  emoji: '🍇', color: '#7d3c98' },
    { id: 'sp-cognac',   type: 'spirit', cat: 'spirits', art: 'cognac',  emoji: '🥃', color: '#935116' },
    // ── HISTORIA ──────────────────────────────────────────────
    { id: 'hist-origins',    type: 'history', cat: 'history', art: 'origins',      emoji: '🏛️', color: '#8e44ad' },
    { id: 'hist-prohibition',type: 'history', cat: 'history', art: 'prohibition',  emoji: '🚫', color: '#8e44ad' },
    { id: 'hist-iba',        type: 'history', cat: 'history', art: 'iba',          emoji: '🏅', color: '#8e44ad' },
    { id: 'hist-tiki',       type: 'history', cat: 'history', art: 'tiki-culture', emoji: '🗿', color: '#8e44ad' },
    // ── TRENDS (AMAROS / BITTERS) ─────────────────────────────
    { id: 'trend-campari',   type: 'trend', cat: 'amaros', art: 'campari',            emoji: '🔴', color: '#c0392b' },
    { id: 'trend-aperol',    type: 'trend', cat: 'amaros', art: 'aperol',             emoji: '🟠', color: '#e67e22' },
    { id: 'trend-fernet',    type: 'trend', cat: 'amaros', art: 'fernet',             emoji: '🖤', color: '#4a235a' },
    { id: 'trend-angostura', type: 'trend', cat: 'amaros', art: 'angostura-bitters',  emoji: '💧', color: '#922b21' },
  ];

  // UTC day-of-epoch — same pattern as the Featured cocktail rotation in
  // js/screens.jsx. Ensures every user sees the same article on a given day
  // regardless of timezone.
  function dayOfEpochUTC() {
    const now = new Date();
    return Math.floor(Date.UTC(
      now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate()
    ) / 86400000);
  }

  // Safe translator that returns undefined if the key is missing instead of
  // echoing the key back — lets resolveArticle() skip absent sections.
  function tr(key) {
    const fn = window.stLang && window.stLang.t;
    if (!fn) return undefined;
    const v = fn(key);
    return (v && v !== key) ? v : undefined;
  }

  function resolveArticle(entry) {
    if (!entry) return null;
    const base = 'wiki.art.' + entry.cat + '.' + entry.art;
    const title = tr(base);
    // If translations haven't loaded yet, skip rendering instead of showing
    // a raw slug like "shake" — Home re-renders on `stirio:langchange`.
    if (!title) return null;
    const sub = tr(base + '.sub');
    const description = tr(base + '.description');
    const history = tr(base + '.history');
    const whenToUse = tr(base + '.when_to_use') || tr(base + '.when');
    const howTo = tr(base + '.how');
    const tips = tr(base + '.tips');
    const origin = tr(base + '.origin');
    const production = tr(base + '.production');

    // Compose ordered sections, skipping any that are missing.
    const sections = [];
    if (description) sections.push({ label: 'article.section.description', text: description });
    if (origin)      sections.push({ label: 'article.section.origin',      text: origin });
    if (history)     sections.push({ label: 'article.section.history',     text: history });
    if (whenToUse)   sections.push({ label: 'article.section.when',        text: whenToUse });
    if (howTo)       sections.push({ label: 'article.section.how',         text: howTo });
    if (production)  sections.push({ label: 'article.section.production',  text: production });
    if (tips)        sections.push({ label: 'article.section.tips',        text: tips });

    // Excerpt: prefer sub, fall back to first sentence of description.
    let excerpt = sub || '';
    if (!excerpt && description) {
      const dot = description.indexOf('. ');
      excerpt = dot > 0 ? description.slice(0, dot + 1) : description;
      if (excerpt.length > 140) excerpt = excerpt.slice(0, 137) + '…';
    }

    return {
      id: entry.id,
      type: entry.type,
      cat: entry.cat,
      art: entry.art,
      title,
      excerpt,
      emoji: entry.emoji,
      color: entry.color,
      sections,
    };
  }

  function getArticleOfTheDay() {
    if (!POOL.length) return null;
    const idx = dayOfEpochUTC() % POOL.length;
    return resolveArticle(POOL[idx]);
  }

  window.stArticles = { getArticleOfTheDay, resolveArticle, POOL };
})();
